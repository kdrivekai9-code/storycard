...
import { generateMask, submitFalInpaint, pollFalInpaint } from "./actions";
...
export function FalInpaintClient() {
  ...
  function startPolling(statusUrl: string, responseUrl: string) {
    ...
    responseUrlRef.current = responseUrl;
    stopPolling();
        ...
        <form ref={formRef} onSubmit={handleSubmit}>
          ...
          <details style={{ marginBottom: 20, border: "1px solid var(--line)", borderRadius: 8, padding: "12px 16px" }}>
            ...
            <div style={{ fontSize: 12, color: "var(--ink-soft)", marginTop: 10, marginBottom: 14, padding: "8px 12px", background: "var(--bg-soft)", borderRadius: 6 }}>
              Flux 계열은 Guidance Scale이 낮아야 자연스럽습니다 (기본 3.5). SD계열(~7.5)과 다릅니다.
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div className="admin-form-group">
                <label className="admin-label">Strength (배경 변화)</label>
                <input name="strength" type="number" min="0" max="1" step="0.05" defaultValue="0.85" disabled={loading}
              ...
              <div className="admin-form-group">
                ...
                <input name="num_inference_steps" type="number" min="10" max="50" defaultValue="28" disabled={loading}
                  style={{ width: "100%", boxSizing: "border-box" }} />
              </div>
              <div className="admin-form-group">
                <label className="admin-label">Seed (비워두면 랜덤)</label>
                <input name="seed" type="number" placeholder="예: 42" disabled={loading}
                  style={{ width: "100%", boxSizing: "border-box" }} />
              </div>
            </div>
          </details>
      ...
      {result && (
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={result} alt="Flux Inpainting 결과" style={{ maxWidth: 400, borderRadius: 8, border: "1px solid var(--line)", display: "block" }} />
